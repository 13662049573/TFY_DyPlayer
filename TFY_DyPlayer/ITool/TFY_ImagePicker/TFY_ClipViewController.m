...
#define picker_iPhoneX ([UIScreen instancesRespondToSelector:@selector(currentMode)] ? ((NSInteger)(([[UIScreen mainScreen] currentMode].size.height/[[UIScreen mainScreen] currentMode].size.width)*100) == 216) : NO)

#define picker_NavBarHeight           (picker_iPhoneX ? 88.0 : 64.0)
...
-(void)CreatUI
{
    ...
    _overView.opaque = NO;
    [_overView setFrame:CGRectMake(0, 0, width, height)];
    [self.view addSubview:_overView];
    
    UIView *views=[[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, picker_NavBarHeight)];
    views.backgroundColor=[UIColor blackColor];
    views.alpha=0.8;
    [self.view addSubview:views];
    
    //
    UIButton * Retirement = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [Retirement setFrame:CGRectMake(20, picker_NavBarHeight/2-12, 50, 44)];
    [Retirement setTitle:@"取消" forState:UIControlStateNormal];
    [Retirement setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    Retirement.titleLabel.font=[UIFont boldSystemFontOfSize:14];
    [Retirement addTarget:self action:@selector(RetirementSelected) forControlEvents:UIControlEventTouchUpInside];
    [views addSubview:Retirement];
    ...
    clipBtn.titleLabel.font=[UIFont boldSystemFontOfSize:16];
    [clipBtn addTarget:self action:@selector(clipBtnSelected:) forControlEvents:UIControlEventTouchUpInside];
    [clipBtn setBackgroundColor:[UIColor colorWithRed:0.10 green:0.58 blue:0.96 alpha:1.00]];
...
-(void)RetirementSelected{
    [self.delegate RetirementController:self];
}

-(void)clipBtnSelected:(UIButton *)btn
{
    [self.delegate ClipViewController:self FinishClipImage:[self getSmallImage] WithName:_imageName];
}
